import { Given, When, Then } from '@wdio/cucumber-framework'; //eslint-disable-line no-unused-vars
...
When(
  'the generate button is clicked and it creates a string',
  async function () {
...
Then(
  'the copy button can be pressed to copy it to the clipboard',
  async function () {
    await ShadowDomPage.copyButton.click();
    const fieldText = await ShadowDomPage.guidField.getValue();
    await expect(
